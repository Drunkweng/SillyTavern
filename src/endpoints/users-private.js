...
router.get('/me', asyncHandler(async (request, response) => {
    ...
    const viewModel = {
        ...
        enableDatabaseAuth: await getConfigValue('enableDatabaseAuth', false, 'boolean'),
    };

    return response.json(viewModel);
}));

router.post('/change-avatar', asyncHandler(async (request, response) => {
...
router.post('/change-password', asyncHandler(async (request, response) => {
    const enableDatabaseAuth = await getConfigValue('enableDatabaseAuth', false, 'boolean');
    if (enableDatabaseAuth) {
        return response.status(403).json({ error: 'Password modification is disabled when using database authentication.' });
    }
    if (!request.body.handle) {
    ...
    if (!request.user.profile.admin && user.password && user.password !== await getPasswordHash(request.body.oldPassword, user.salt)) {
...
router.post('/backup', asyncHandler(async (request, response) => {
...
router.post('/reset-settings', asyncHandler(async (request, response) => {
    const password = request.body.password;

    if (request.user.profile.password && request.user.profile.password !== await getPasswordHash(password, request.user.profile.salt)) {
...
router.post('/change-name', asyncHandler(async (request, response) => {
...
router.post('/reset-step1', asyncHandler(async (request, response) => {
...
router.post('/reset-step2', asyncHandler(async (request, response) => {
    if (!request.body.code) {
        console.warn('Recover step 2 failed: Missing required fields');
        return response.status(400).json({ error: 'Missing required fields' });
    }

    if (request.user.profile.password && request.user.profile.password !== await getPasswordHash(request.body.password, request.user.profile.salt)) {
